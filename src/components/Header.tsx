...
import { Menu, X, Home, Heart, Film, Sparkles } from 'lucide-react';
...
const Header: React.FC = () => {
    ...
    const navLinks = [
        { to: '/', label: 'Home', icon: Home, activeIcon: Sparkles },
        { to: '/favorites', label: 'My List', icon: Heart, showBadge: true },
    ];
                ...
                <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-2 group">
                        <div className="relative p-1.5 rounded-lg bg-linear-to-br from-rose-500/20 to-orange-500/20 group-hover:from-rose-500/30 group-hover:to-orange-500/30 transition-all duration-300">
                            <Film size={24} className="text-rose-500 group-hover:text-rose-400 transition-colors" />
                        </div>
                        ...
                        {navLinks.map((link) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon
                                        size={16}
                                        className={`transition-all duration-200 ${isActive ? 'text-rose-500 scale-110' : 'hover:scale-110'
                                            } ${link.showBadge && favorites.length > 0 ? 'relative' : ''}`}
                                    />
                                    {link.label}
                                    {isActive && (
